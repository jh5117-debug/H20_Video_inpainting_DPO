...
def build_stage2_cmd(project_root, args):
    ...
    latest_stage1_weights = latest_dir(
        project_root, "sft", "stage1", args.experiments_dir
    ) / "converted_weights"
    pretrained_stage1 = args.pretrained_stage1 or first_existing(
        latest_stage1_weights,
        os.path.join(project_root, "finetune-stage1", "converted_weights"),
    )

    # Validation uses a separate eval dataset, isolated from train data.
    ...
    return cmd, data_dir, weights_dir, pretrained_stage1, str(output_dir)
...
def run_stage2(args=None):
    ...
    cmd, data_dir, weights_dir, pretrained_stage1, output_dir = build_stage2_cmd(project_root, args)

...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments/sft/stage1/manual",
    ...
    parser.add_argument(
        "--pretrained_stage1_path",
        type=str,
        default=None,
        help="Path to pretrained stage1 weights (containing unet_main/ and brushnet/ subdirs) for finetuning.",
    )
    parser.add_argument(
        "--csv_file",
        type=str,
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        if args.report_to == "wandb":
            init_kwargs["wandb"] = {
                "name": f"stage1-{args.max_train_steps}steps",
            }
                        ...
                        try:
                            ...
                            if is_wandb_available():
                                artifact = wandb.Artifact(
                                    f"stage1-weights-step{global_step}",
                                    type="model",
                                    metadata={"global_step": global_step, "loss": loss.detach().item()}
                                )
                                artifact.add_dir(converted_dir)
                                wandb.log_artifact(artifact)
                                logger.info(f"Uploaded W&B Artifact: stage1-weights-step{global_step}")
                        except Exception as e:
    ...
    if accelerator.is_main_process:
        # --- Save final weights (Stage 1: UNet2D + BrushNet) ---
        ...
        logger.info(f"Stage 1 converted weights saved to {converted_dir}")
